            ...
            if ftd == 1:
                ...
                if td_flag == 0:
                    sql2 = """
                                    	...
                                    	""" % (chk_db, chk_tb, tb_name, pro_name, dodate, table_rows, ut_val)
                else:
                    sql2 = """
                        insert into %s.%s set table_name='%s',pro_name='%s',view_date='%s',table_rows='%s',last_update_time='%s',last_trade_date='%s'
                        """ % (chk_db, chk_tb, tb_name, pro_name, dodate, table_rows, ut_val, td_val)
            else:
                if td_flag == 0 :
                    sql2 = """
                    ...
                    sql2 = """
                        ...
                        """ % (chk_db, chk_tb, tb_name, pro_name, dodate, table_rows, td_val)
            ai_cur.execute(sql2)
        except:
            pass
